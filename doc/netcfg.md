...
1. **连接设备**：只有「设备地址」一个输入框；右侧并排「测试连接」和红色实心的
   「一键恢复网络」。后者是现场重点工具，用醒目按钮而不是文字链。
   端口、用户名、密码不在界面上暴露，一律取自配置（见下面的「默认配置」），
   要改得改配置再重新构建。打开页面会自动连一次；改了地址之后再点「测试连接」。

   连接区下方有一条**固定高度的状态栏**：配置告警与操作结果共用，有无文案高度不变，
   下方网口区不会因点按钮而上下跳。配置告警存在时优先显示，操作清不掉它。
