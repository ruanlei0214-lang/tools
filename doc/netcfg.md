...
页面按两步排：**1 选择网口 → 2 修改地址**。顶部另有一行操作按钮，不占步骤号。

1. **刷新与恢复**：一行里是「刷新网络配置」、红色实心的「一键恢复网络」、状态。
   地址不再在本页重复显示，用的是顶栏 / `toolbox-config.json` 里那份。
   「一键恢复网络」是现场重点工具，用醒目按钮而不是文字链。
   地址、用户名、密码一律取自共享配置 `toolbox-config.json`（exe 同目录，
   全系列工具共用），界面上不暴露编辑。点「刷新网络配置」前不用先点顶栏「连接」——
...
2. **选择网口**：刷新成功会把网口读回来。五个面板口（`lan1`~`lan5`）**竖排成列表**，
...
| 字段 | 作用 |
| --- | --- |
| `device.host` | 共享配置里没有地址时的出厂值；界面上只读，改地址去 `toolbox-config.json` |
