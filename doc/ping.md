...
当前版本 **V1.1.3**，声明在 `frontend/src/modules/ping/module.ts`。
...
切到「扫描网段」页签或别的模块都不会掐断长 ping：模块切换走 keep-alive，
组件是停用不是卸载，长 ping 在后台照跑、日志照攒（缓冲区 500 行上限），
切回来接着看。只有整个视图被销毁（程序退出）时才会把后端的 ping 停掉。
...
**扫到一个显示一个**：扫描途中每 ping 通一台，后端就往前端推一帧
（`ping:scan-found` 事件），表格按 IP 顺序插入新行，状态栏同步报「已发现 N 台」。
推来的行只有 IP 和延迟；扫完 `Scan` 返回完整结果，整体替换表格，补上设备名和 MAC，
状态栏报「在线 N 台 / 共扫 M 个地址，用时 Xs」。设备名和 MAC 拿不到时显示「—」。
