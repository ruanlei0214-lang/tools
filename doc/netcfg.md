...
当前版本 **V1.0.1**，声明在 `frontend/src/modules/netcfg/module.ts`。
...
1. **设备连接**：界面上只有「设备地址」一个输入框。端口、用户名、密码不在界面上暴露，
   一律取自配置（见下面的「默认配置」），要改得改配置再重新构建。
   打开页面会自动连一次，不用每次手点；改了地址之后点「测试连接」重新验证。
2. **看网口**：连接成功会顺带把网口读回来，不用单独再点一次。表格里是**机柜面板上印的
   网口名**（`lan1`~`lan5`），不是系统网口名——现场对着机柜找口，只认面板丝印。
   每行显示状态、IP、掩码、网关、MAC。点任意一行选中它，下方表单会自动填入当前配置。

   **只有落在设备主网口上的面板口能改地址**，其余的带「只读」标记、点不动，
   但信息照常显示。表格上方会列出当前哪几个口可改。

   面板 `lan3` 恒定是空行：它不由本工具管理。不可选中，但保留占位，
   免得表格里凭空少一个口让人以为读漏了。对应关系见下面的「面板网口对应关系」。

   连接失败时表格会被清空。留着上一次的网口列表会让人以为那是当前设备的状态，
   照着它改地址就改到别处去了。
3. **修改地址**：改 IP、掩码、网关（网关留空表示不动默认路由），点「下发配置」。
   按钮会变成「确认下发」要求二次确认，因为填错网段等于把设备从网络上摘掉。

   选中的面板口如果和别的面板口共用同一个系统网口（桥接形态下 `lan1`/`lan2`/`lan5`
   都是 `br0`），标题下方会写明「改一个另外几个跟着一起变」。

   改的如果落在 `persistIface`（默认 `br0`）上，地址会同时写进 `restoreFile` 做持久化，
   **重启后仍然生效**；改其他网口只改运行时地址，重启就没了。按钮旁的提示会说明
   当前选中的网口属于哪一种，不用记。
4. 下发成功后，连接地址会自动切换到新 IP，可以直接再点「测试连接」确认。
5. **一键恢复网络**：填好设备连接参数后点「一键恢复网络」，删除配置里 `restoreFile`
   指向的文件（默认 `/opt/runtime/pi`）。删除成功会弹窗提示需要重启机器人控制器，
   点「知道了」关掉，重启要自己去做，工具不代劳。

   这个按钮不做二次确认——按钮叫「一键」，中间再插一次确认就名不副实了。
   代价是误触会直接删文件，所以结果提示里写明了删掉的是哪个路径。

## 面板网口对应关系

机柜面板上印的网口名和系统里的网口名对不上，而现场只认面板丝印，所以表格按面板展示，
...
表格上方会把当前可改的口列出来，不用对着上面那张表推。

「只读」和「空占位」是两种状态，表格上分得开：只读行有完整信息、名字后面带「只读」
标记；占位行整行是 `—` 且整体压暗。代码里对应 `Port.Editable` 与 `Port.Iface == ""`。

判断「哪个是主网口」用的还是 `bridgeIface`/`mainIface` 两个常量，和面板对应关系共用
同一套判断，不会出现「表格按有桥来排、可改性按没桥来判」的分裂。这条规则同样由设备侧
决定，写死在 `ports.go`，不做成配置。

## 默认配置

页面打开时各输入框的初始值、连接超时，以及一键恢复要删除的路径，都来自
`internal/modules/netcfg/config.json`：
...
| 字段 | 作用 |
| --- | --- |
| `device.host` | 「设备地址」输入框的初始值，界面上可以改 |
| `device.port` / `user` / `password` | SSH 连接用的端口、账号、密码，界面上不显示、改不了；`port` 写 0 或省略按 22 处理 |
...
「设备地址」有个例外：如果之前连通过某个地址，下次打开页面填的是**那个地址**，而不是
配置里的出厂地址。设备改完 IP 之后不用每次重新输入。

记录存在 `%APPDATA%\embedtools\netcfg-state.json`，只存一个 `host` 字段。只有确认连得通
才会记下来——「测试连接」成功（连接与读网口两步各记一次），以及「下发配置」成功（这时
记的是下发的新地址，因为设备接下来在那个地址上）。连不上的地址不会被记住，所以填错一次
不会污染下次打开的初始值。

想回到配置里的出厂地址，删掉这个文件即可。

<details>
<summary>为什么存 %APPDATA% 而不是 exe 旁边</summary>

exe 可能被放在 Program Files 或只读共享盘上，那里写不进去。代价是把 exe 拷到另一台机器
不会带走这份记录，但它本来就只是个便利，丢了只是退回出厂地址。

同理，这条路径上的任何失败（目录建不出来、文件损坏、写不进去）都不往界面上报，只写日志：
...
**只有设备地址留在界面上。** 端口、用户名、密码在现场基本不变，进了配置之后再摆三个
输入框只是噪音，所以界面上不再显示它们。代价是现场换个账号必须重新构建；判断得失的
依据是这三项的变动频率远低于设备地址。连接失败时的错误消息里带着 `host:port`，
排查时还能看到实际用的端口。
...
| `Defaults` | `() => Promise<Settings>` | 返回页面默认值；地址优先用上次连通过的那个 |
...
**面板对应关系写死，只适用当前机型。** 换一台接线不同的设备，表格会指错口——而且不会
...
**只显示每个网口的第一个 IPv4 地址。** 一个网口配了多个地址时，表格只展示第一个，
...
internal/modules/netcfg/config.json     默认配置，编译进产物，改完要重新构建
