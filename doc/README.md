...
| 模块 | 版本 | 文档 | 说明 |
| --- | --- | --- | --- |
| remote | V1.4.1 | [remote.md](remote.md) | 控制上位机 IO 与寄存器，点位和连接参数在界面上改；指令页可重启控制器 |
...
| ping | V1.1.3 | [ping.md](ping.md) | 侧栏「网络检测」：长 ping 与网段扫描（IP、设备名、MAC） |
...
名称、标识、版本、一句说明和几条简要操作（来自模块 `module.ts` 里的 `guide`，
一行一条，只写最容易问出口的那几步）。切到别的模块再打开，第二组整组跟着变。
