...
当前版本 **V1.0.4**，声明在 `frontend/src/modules/netcfg/module.ts`。
...
1. **连接设备**：**整块压成一行**——标题、「设备地址」输入框、「测试连接」、红色实心的
   「一键恢复网络」、状态依次排开（三个模块的连接区都是这个样子）。
   「一键恢复网络」是现场重点工具，用醒目按钮而不是文字链。
   端口、用户名、密码不在界面上暴露，一律取自配置（见下面的「默认配置」），
   要改得改配置再重新构建。填好地址后自己点「测试连接」，页面不会自动连——
   开机时设备还没起来，一打开就连会把程序卡住。改了地址之后也要再点一次。
...
记录存在 exe 同目录的 `netcfg-state.json`，只存一个 `host` 字段。只有确认连得通
