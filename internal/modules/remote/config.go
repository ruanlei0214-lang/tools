...
func validatePanel(tab Tab) (Tab, panelSource, error) {
	...
	if !ok {
		return Tab{}, panelSource{}, fmt.Errorf("不认识的标签页类型 %q，只支持 %s、%s",
			tab.Kind, kindIO, kindRegister)
	}
	tab.Kind = src.kind
...
func normalizeTab(t *Tab, seen map[string]bool) error {
	...
	default:
		return fmt.Errorf("标签页 %q 的 kind %q 不认识，只支持 %s、%s", t.ID, t.Kind, kindIO, kindRegister)
	}
